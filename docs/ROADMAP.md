...
| 5 — Nav/Corp/Content | ✅ Mostly complete | Built: routing + avoidance + waypoint, courier, sov-owner overlay, **WH mass roller** + **signature/chain tracker** + Thera/EVE-Scout, fleet read + **MOTD/free-move write**, in-game UI bridge, jump-fatigue, corp members/structures/**SRP**/**recruitment**/**timerboard**, PvE incursions/FW/**abyss tracker**, LP, calendar. Residual (niche / needs-live): ADM figures, agent/mission finder, moon scheduling, member move/invite (wing/squad), container-log theft. (Bookmark mgr removed — CCP retired the ESI scope.) |
...
| 6.5 — AI (Jarvis) | ✅ Mostly complete | Built: OpenAI-compatible local client + auto-detect, tool registry + orchestrator, **per-domain specialist agents**, proactive briefing, durable memory + **retrieval-augmented recall**, **MCP server core** (JSON-RPC handler + dispatch command). Residual (needs infra): MCP stdio/HTTP transport bridge, **sqlite-vec vector RAG** (embeddings model), **voice** (whisper/Piper binaries). |
